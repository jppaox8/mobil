// Shared React UI components for Arabella Chic.
//
// Loaded as <script type="text/babel" src="js/components.js"> AFTER js/utils.js
// and BEFORE the page script. Business logic lives in js/utils.js (exposed as
// `ArabellaUtils` / `window.formatPrice`); this file only holds presentation.
//
// babel-standalone runs each script in global scope, so the components are
// attached to `window` to be reachable from the page scripts.
